...
type ProbMap = mutable.Map[(Class[_], Option[Any]), Double]
...
object ProbUpdate {
  ...
  var fitMap: ProbMap = mutable.Map[(Class[_], Option[Any]), Double]()
  var probMap: ProbMap = mutable.Map[(Class[_], Option[Any]), Double]()
  ...
  def getAllNodeTypes(program: ASTNode): Set[(Class[_], Option[Any])] = {
    val recurseValue =
      if (
        program.isInstanceOf[StringLiteral] || program
          .isInstanceOf[IntLiteral] || program.isInstanceOf[StringVariable]
        || program.isInstanceOf[IntVariable] || program
          .isInstanceOf[StringLiteral] || program
          .isInstanceOf[BoolLiteral] || program.isInstanceOf[BoolVariable]
        || program.isInstanceOf[BVLiteral] || program.isInstanceOf[BVVariable]
      )
        (program.getClass -> Some(program.code))
      else (program.getClass -> None)
    program.children.flatMap(c => getAllNodeTypes(c)).toSet + recurseValue
  }

  /** def update(fitsMap: mutable.Map[(Class[_], Option[Any]), Double],
    * currLevelProgs: mutable.ArrayBuffer[ASTNode], task: SygusFileTask):
    * mutable.Map[(Class[_], Option[Any]), Double] = { fitMap = fitsMap val
    * examples = ListBuffer[Set[Any]]() for (program <- currLevelProgs) { val
    * exampleFit = task.fit(program) val fit: Double = (exampleFit._1.toFloat) /
    * exampleFit._2 if (fit > 0) { val examplesPassed = task.fitExs(program)
    * examples += examplesPassed cacheCost += (examplesPassed -> program) } } if
    * (!examples.isEmpty) { val theExample =
    * examples.sortBy(_.toList.length).last if (!theExample.isEmpty) { if
    * (currBest.toList.length < theExample.toList.length) { currBest =
    * theExample val prog = cacheCost(theExample) //println(prog.code,
    * theExample, prog.cost) val changed: Set[(Class[_], Option[Any])] =
    * getAllNodeTypes(prog) for (changedNode <- changed) { if
    * (!fitMap.contains(changedNode) || fitMap(changedNode) > (1 -
    * theExample.toList.length)) { val update = expo(probMap(changedNode), (1-
    * theExample.toList.length)) fitMap += (changedNode -> update) probMap +=
    * (changedNode -> update) } } } } } fitMap }
    */
  ...
  def readjustCosts(task: SygusFileTask): Unit = this.synchronized {
    ...
    for (program <- cache.keys) {
      ...
      if (fit > 0) {
        ...
        if (!cacheExamples.contains(examplesPassed)) {
          ...
          for (changedNode <- changed) {
            if (
              !fitMap.contains(changedNode) || fitMap(changedNode) > (1 - fit)
            ) {
      ...
      /** val fit = (cache(ast.code).toFloat) / task.examples.length for
        * (changedNode <- changed) { Console.withOut(fos) { (task.examples) } if
        * (!localMap.contains(changedNode) || localMap(changedNode) > (1 - fit))
        * { val update = expo(ProbUpdate.probMap(changedNode), (1 - fit))
        * localMap += (changedNode -> update) probMap += (changedNode -> update)
        * } } }
        */
    }
  }
  ...
  def update(
      fitsMap: mutable.Map[(Class[_], Option[Any]), Double],
      currLevelProgs: mutable.ArrayBuffer[ASTNode],
      task: SygusFileTask
  ): mutable.Map[(Class[_], Option[Any]), Double] = {
    fitMap = fitsMap
  ...
  def updatePriors(
      pMap: mutable.Map[(Class[_], Option[Any]), Double],
      round: Int
  ): mutable.Map[(Class[_], Option[Any]), Int] = {
  ...
  def getRootPrior(node: ASTNode): Int = if (
    node.isInstanceOf[StringLiteral] || node
      .isInstanceOf[StringVariable] || node.isInstanceOf[IntVariable]
    || node.isInstanceOf[IntLiteral] || node.isInstanceOf[BoolVariable] || node
      .isInstanceOf[BoolLiteral]
  ...
  else if (num - num.toInt > 0.5) math.ceil(num).toInt
  else math.floor(num).toInt
  ...
  def createPrior(
    ...
    vocab.leavesMakers.map(l =>
      priors += ((l.nodeType, Some(l.head)) -> createRoundValue(
        -log2(probMap((l.nodeType, Some(l.head))))
      ))
    )
    vocab.nodeMakers.map(l =>
      priors += ((l.nodeType, None) -> createRoundValue(
        -log2(probMap((l.nodeType, None)))
      ))
    )
    priors
  }
  ...
  def createProbMap(
    ...
    vocab.leavesMakers.map(l =>
      probMap += ((l.nodeType, Some(l.head)) -> uniform)
    )
